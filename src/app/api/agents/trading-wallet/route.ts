...
// ── GET /api/agents/trading-wallet?userAddress=0x... ─────────────────────────
// Returns Trading Wallet provision status and USDC balance for a user.
...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      engineRes = await fetch(`${ENGINE_URL}/agents/trading-wallet?userAddress=${encodeURIComponent(userAddress)}`, {
        method: 'GET',
...
// ── POST /api/agents/trading-wallet ──────────────────────────────────────────
// Withdraws USDC from Trading Wallet to specified destination address (or default Agent Wallet).
