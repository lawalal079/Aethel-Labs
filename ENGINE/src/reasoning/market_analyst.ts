...
import { fetchBTCCandles, syntheticCandle, type OHLCCandle } from '../lib/ohlc-feed';
...
export async function runMarketAnalystCycle(agentId: string = 'smc_alpha_executor'): Promise<SharedDecision | null> {
  ...
  // 2. Fetch real OHLC candles from CoinGecko (1 call per interval for ALL users)
  _totalAnalystCoinGeckoCalls++;
  const btcCandles = (await fetchBTCCandles(1)) ?? [syntheticCandle(validBtcUsd)];
  ...
  const smcCtx: SMCContext = {
    ...
    candles: btcCandles.length > 0 ? btcCandles : [syntheticCandle(validBtcUsd)],
  };
